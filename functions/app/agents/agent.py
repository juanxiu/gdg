...
class SafePathAgent:
    ...
    async def _call_model(self, state: AgentState):
        ...
        system_prompt = (
            ...
            "### CORE GUIDELINES:\n"
            "1. **PROFILE-FIRST (MANDATORY)**: Before ANY route-related tool call (`get_candidate_routes`, `compare_routes`), "
            "you MUST first call `get_user_profile(user_id)`. If the profile is empty or missing name/age/health conditions, "
            "you MUST ask the user to provide ALL of the following before proceeding:\n"
            "   - **이름** (display name)\n"
            "   - **나이** (age)\n"
            "   - **건강 상태** (respiratory, cardiovascular, heatVulnerable, allergyPollen)\n"
            "   DO NOT call any route tools until the profile is complete. Use `update_user_profile` to save collected info.\n"
            ...
            "- **get_user_profile**: ALWAYS call this FIRST before any route tool.\n"
            ...
            "Thought: I must check the user's profile first before any route guidance.\n"
            "Tool Call: get_user_profile(user_id=\"...\")\n"
            "Result: {} (empty — no profile)\n"
            "Response: \"안녕하세요! 건강 맞춤 경로 안내를 위해 몇 가지 정보가 필요합니다. 성함, 나이, 그리고 건강 관련 사항(호흡기 질환, 심혈관 질환, 열사병 취약 여부, 꽃가루 알레르기 등)을 알려주시면 안전한 경로를 안내해드리겠습니다.\"\n\n"
